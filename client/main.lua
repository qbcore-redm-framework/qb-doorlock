				...
				if doorID.doors then
					if doorID.locked then
						for _,v in ipairs(doorID.doors) do
							if DoorSystemGetDoorState(v.doorid) ~= 3 then
								AddDoorToSystemNew(v.doorid,1,1,0,0,0,0)
								DoorSystemSetDoorState(v.doorid, 3)
							end
						...
						for _,v in ipairs(doorID.doors) do
							if DoorSystemGetDoorState(v.doorid) ~= false then
								AddDoorToSystemNew(v.doorid,1,1,0,0,0,0)
								DoorSystemSetDoorState(v.doorid, 0)
							end
						end
					...
					if doorID.locked then
						if DoorSystemGetDoorState(doorID.doorid) ~= 3 then
							AddDoorToSystemNew(doorID.doorid,1,1,0,0,0,0)
							DoorSystemSetDoorState(doorID.doorid, 3)
						end
						...
						if DoorSystemGetDoorState(doorID.doorid) ~= false then
							AddDoorToSystemNew(doorID.doorid,1,1,0,0,0,0)
							DoorSystemSetDoorState(doorID.doorid, 0)
						end
						FreezeEntityPosition(doorID.object,false)
					end
				end
